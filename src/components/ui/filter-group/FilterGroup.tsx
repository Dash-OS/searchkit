...
import { ReactComponentType, PureRender, FastClick } from '../../../';
...
@PureRender
export class FilterGroupItem extends React.Component<
  ...
  render() {
    ...
    return (
      <FastClick handler={this.removeFilter}>
        <div className={bemBlocks.items('value').toString()} data-key={itemKey}>
          {label}
        </div>
      </FastClick>
    );
  ...
  renderRemove(bemBlocks) {
    ...
    return (
      <FastClick handler={this.removeFilters}>
        <div
          className={bemBlocks.container('remove-action').toString()}
