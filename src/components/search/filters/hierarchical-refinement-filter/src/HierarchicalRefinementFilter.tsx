...
export interface HierarchicalRefinementFilterProps
  extends SearchkitComponentProps {
  field: string;
  id: string;
  title: string;
  size?: number;
...
export class HierarchicalRefinementFilter extends SearchkitComponent<
  HierarchicalRefinementFilterProps,
  any
> {
      ...
      <div key={option.key}>
        <FastClick handler={this.addFilter.bind(this, level, option)}>
          <div className={className}>
            <div className={block('text').toString()}>
              {this.translate(option.key)}
            </div>
            <div className={block('count').toString()}>
  ...
  renderOptions(level) {
    ...
    return (
      <div className={block('hierarchical-options').toString()}>
        {map(
          this.accessor.getBuckets(level),
          this.renderOption.bind(this, level),
        )}
      </div>
    );
  }
  ...
  render() {
    ...
    return (
      <div data-qa={`filter--${this.props.id}`} className={className}>
        <div data-qa="title" className={block('header').toString()}>
          {this.props.title}
        </div>
